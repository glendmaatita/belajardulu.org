...
export function SimulatorProporsi() {
  ...
  const Step = ({ label, val, set, step }: { label: string; val: number; set: (n: number) => void; step: number }) => (
    <div className="flex items-center gap-2">
      ...
      <button onClick={() => set(Math.max(step, val - step))} className="h-7 w-7 rounded-md bg-canvas font-bold text-ink-soft hover:bg-line">-</button>
      <span className="w-16 text-center font-bold tnum">{fmt(val)}</span>
      <button onClick={() => set(val + step)} className="h-7 w-7 rounded-md bg-canvas font-bold text-ink-soft hover:bg-line">+</button>
    </div>
  );
