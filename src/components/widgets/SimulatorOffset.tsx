      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block">
          ...
          <select
            ...
            className="w-full rounded-lg border border-line-strong px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none"
          >
            {PROYEK.map((x) => (
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
        checked ? "border-emerald-400 bg-emerald-50 text-emerald-700" : "border-line bg-white text-ink-faint"
      }`}
    >
      <span className={`grid h-4 w-4 place-items-center rounded ${checked ? "bg-emerald-500 text-white" : "bg-line"}`}>
        {checked ? "✓" : ""}
      </span>
      {label}
    </button>
