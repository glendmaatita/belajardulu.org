...
This is **BelajarYuk**, an Indonesian interactive learning platform. The product is a client-rendered React 19 app built with Vite, React Router 7, Tailwind CSS 3, Remotion 4, and an Express + better-sqlite3 backend.

Key architectural constraints:
- Lesson content is **data-driven**, stored as `ContentBlock[]` arrays under `src/topics/<topic>/lessons/`, not hand-written JSX
- Each topic contains levels, each level contains lessons; each lesson includes exercises, quizzes, charts, videos, and simulators
- The platform currently supports topics such as accounting, carbon trading, finance, math, and philosophy
- User-facing copy must be in **Bahasa Indonesia** and must never use em-dash (`—`)
